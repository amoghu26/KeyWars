...
import javax.swing.*; // This code is the second level of the game, this class is where everything in level two is 


public class LVL2 extends JPanel implements KeyListener { // Panel where everything is drawn and all code is written 
	static JPanel cards = StartPanel.cards;
	private int playerX, playerY; // coordinates of the player
	private int aX, aY, bX, bY, cX, cY; //coordinates of the guards
	...
	int bposition3;
	int bposition4;
	boolean amovement = true, bmovement = true, cmovement = true, upshoot = false, downshoot = false, leftshoot = false, rightshoot = false, guardADead= false, guardBDead = false, guardCDead = false, playerAlive = true, blocks = true; // determines which way the guards move
	long time;
	long end;
	...
	boolean randomcheck = false, TNTBlownUp = false, TNT2BlownUp, TNT3BlownUp, TNT4BlownUp; // checks if TNT is intact
	int TNTx, TNTy; // TNT randomized coordinates
	int tnt2x, tnt2y, tnt3y, tnt3x, tnt4x, tnt4y;
	boolean tnt2check = false, tnt3check = false, tnt4check = false;
	...
	public void paintComponent(Graphics g) { //draws and paints the players, guards, key, and the background 
		...
		Rectangle key = new Rectangle(1370, 200, 50, 50);
		...
		if(guardADead == false && blocks == true)
		{
		...
		if(guardBDead == false && blocks == true)
		{
			g.setFont(new Font("TimesRoman", Font.PLAIN, 15)); 
		...
		if(guardCDead == false && blocks == true)
		{
			g.setFont(new Font("TimesRoman", Font.PLAIN, 15)); 
		...
		if(tnt3check == false)
		{
			...
			while(tnt3y>720 || tnt3y<100 || (tnt3y>TNTy-50 && tnt3y<TNTy+50) || (tnt3y>speedy - 50 && tnt3y<speedy+50) || (tnt3y>bullety - 50 && tnt3y<bullety+50) || (tnt3y>mapy-50 && tnt3y<mapy+50) && (tnt3y>tnt2y-50 && tnt3y<tnt2y+50))
			{
				tnt3y = (int)(Math.random()*851) + 0;
			}
			tnt3check = true;
		...
		if(tnt4check == false)
		{
			...
			while(tnt4y>720 || tnt4y<100 || (tnt4y>TNTy-50 && tnt4y<TNTy+50) || (tnt4y>speedy - 50 && tnt4y<speedy+50) || (tnt4y>bullety - 50 && tnt4y<bullety+50) || (tnt4y>mapy-50 && tnt4y<mapy+50) && (tnt4y>tnt2y-50 && tnt4y<tnt2y+50))
			{
				tnt4y = (int)(Math.random()*851) + 0;
			}
			tnt4check = true;
		...
		if (guardCDead == false && blocks == true) g.fillRect(cX+10, cY-35, 100, 100); 
		if (guardCDead == false && blocks == true && cmovement == true) g.drawImage(guardBImage, cX+10, cY-35, this);
		if (guardCDead == false && blocks == true && cmovement == false) g.drawImage(guardAImage, cX+10, cY-35, this);
		
		g.setColor(Color.YELLOW);
		
		Image keyimage = new ImageIcon("key.png").getImage();
		if((blocks == true && guardADead == true && guardBDead == true && guardCDead == true) || (blocks == true && mapactivate == true)) g.fillRect(1370, 200, 50, 50);
		if((blocks == true && guardADead == true && guardBDead == true && guardCDead == true) || (blocks == true && mapactivate == true)) g.drawImage(keyimage, 1370, 200, this);
		...
		if(player.intersects(key) && guardADead == true && guardBDead == true && dead == false || player.intersects(key) && mapactivate == true && dead == false) // checking to see whether the player has interacted with the key or not
		{
			...
			g.setColor(Color.BLACK);
			g.setFont(new Font("TimesRoman", Font.PLAIN, 150)); 
			g.drawString("Passed Level 2 !", 240, 350);
