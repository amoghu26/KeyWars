...
import javax.swing.*; // This code is the third level of the game, this class is where everything in level three is 


public class LVL3 extends JPanel implements KeyListener { // Panel where everything is drawn and all code is written 
	...
	boolean dead = false;
	int ahealth = 500, bhealth = 350, chealth =500, dhealth = 350;;
	boolean healtha = false, healthb = false, healthc = false, healthc2 = false, healthc3 = false;
	
	public LVL3() { // initializing variables
		playerX = 645;
		playerY = 780;
		aX = 10;
		aY = 320;
	...
	class AMover implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if(cX == 1320) // determines the way the third guard moves
			{
				cmovement = true;
			}
			else if(cX == -10)
			...
			if (dX == 1320) //determines which way the fourth guard moves
			{
				dmovement = false;
			}
			else if(dX == -10)
	...
	public void paintComponent(Graphics g) { //draws and paints the players, guards, key, and the background 
		...
		Rectangle key = new Rectangle(50, 400, 50, 50);
		...
		if(guardDDead == false && blocks == true)
		{
			g.setFont(new Font("TimesRoman", Font.PLAIN, 15)); 
		...
		if (guardDDead == false && blocks == true) g.fillRect(dX, dY, 100, 100); 
		if (guardDDead == false && blocks == true && dmovement == true) g.drawImage(guardAImage, dX, dY, this);
		if (guardDDead == false && blocks == true && dmovement == false) g.drawImage(guardBImage, dX, dY, this);
		
		g.setColor(Color.YELLOW);
		
		Image keyimage = new ImageIcon("key.png").getImage();
		if((blocks == true && guardADead == true && guardBDead == true && guardCDead == true && guardDDead == true) || (blocks == true && mapactivate == true)) g.fillRect(50, 400, 50, 50);
		if((blocks == true && guardADead == true && guardBDead == true && guardCDead == true && guardDDead == true) || (blocks == true && mapactivate == true)) g.drawImage(keyimage, 50, 400, this);
		
		if(player.intersects(key) && guardADead == true && guardBDead == true && guardCDead == true && guardDDead == true && dead == false 
				|| player.intersects(key) && mapactivate == true && dead == false) // checking to see whether the player has interacted with the key or not
		{
			playerAlive = false;
			blocks = false;
			...
			g.setColor(Color.BLACK);
			g.setFont(new Font("TimesRoman", Font.PLAIN, 150)); 
			g.drawString("Passed Level 3 !", 240, 350);
